...
bool LED2status = LOW;
//
// for dimmable LED (from ChatGPT)
uint8_t led2Intensity = 125;
// uint8_t lastLed2Intensity = 125;
//
...
void setup() {
  ...
  server.on("/led2intensity", HTTP_GET, handle_led2intensity);  // for dimmable LED (from ChatGPT)
...
void loop() {
  ...
  if (iloops == 0) {       // first time through write this to screen
    ...
    // analogWrite(LED2pin, 0); // first time through the loop confirm LED2 is off regardless of initial intensity value
  ...
  if (LED2status) {
    // for dimmable LED (from ChatGPT)
    analogWrite(LED2pin, led2Intensity);  // Use analogWrite for PWM control
  } else {
    analogWrite(LED2pin, 0);  // Use analogWrite for PWM control
  }
  //
...
void handle_OnConnect() {
  ...
  server.send(200, "text/html", SendHTML(LED1status, LED2status, led2Intensity));
}

void handle_led1on() {
  LED1status = HIGH;
  Serial.println("GPIO4 Status: ON");
  server.send(200, "text/html", SendHTML(true, LED2status, led2Intensity));
}

void handle_led1off() {
  LED1status = LOW;
  Serial.println("GPIO4 Status: OFF");
  server.send(200, "text/html", SendHTML(false, LED2status, led2Intensity));
}
//
// for dimmable LED (from ChatGPT)
void handle_led2on() {
  ...
  server.send(200, "text/html", SendHTML(LED1status, true, led2Intensity));
}

void handle_led2off() {
  LED2status = LOW;
  Serial.println("GPIO5 Status: OFF");
  server.send(200, "text/html", SendHTML(LED1status, false, led2Intensity));
}
...
void handle_led2intensity() {
  String intensityValue = server.arg("value");
  Serial.print("Received intensity value: ");
  Serial.println(intensityValue);
  led2Intensity = intensityValue.toInt();  // Update led2Intensity with the received Value
  if (LED2status) {
    analogWrite(LED2pin, led2Intensity);  // only write to LED2pin if LED2 is on otherwise just save the intensity value for when it is turned on
  }
  server.send(200, "text/html", SendHTML(LED1status, LED2status, led2Intensity));
}
...
// define HTML string
// for dimmable LED (from ChatGPT)
String SendHTML(uint8_t led1stat, uint8_t led2stat, uint8_t led2Intensity) {
  ...
  // for dimmable LED (from ChatGPT)
  // Add this inside the <body> tag of your HTML code
  ptr += "<input type=\"range\" id=\"led2IntensitySlider\" min=\"0\" max=\"255\" value=\"" + String(led2Intensity) + "\" onchange=\"updateIntensity(this.value)\">\n";
  ptr += "<p>LED2 Intensity: <span id=\"led2Intensity\">" + String(led2Intensity) + "</span></p>\n";
  ptr += "<script>\n";
  ptr += "var isLed2On = " + String(LED2status ? "true" : "false") + ";\n";
  ptr += "function updateIntensity(value) {\n";
  ptr += "  document.getElementById(\"led2Intensity\").innerHTML = value;\n";
  ptr += "  var xhttp = new XMLHttpRequest();\n";
  ptr += "  xhttp.open(\"GET\", \"/led2intensity?value=\" + value, true);\n";
  ptr += "  xhttp.send();\n";
  ptr += "}\n";
  ptr += "</script>\n";
  ptr += "</body>\n";
